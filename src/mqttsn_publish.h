...
namespace MQTTSN 
{
    ...
    pack_publish(uint8_t* buffer, uint16_t buffer_len, 
                 uint8_t dup, uint8_t qos, uint8_t retain, uint16_t msg_id, const uint8_t* data, uint16_t data_len);

    /**
     * Unpack the Publish message
     */
    uint8_t 
    unpack_publish(uint8_t* dup, uint8_t* qos, uint8_t* retain, uint16_t* msg_id, const uint8_t* data, uint16_t* data_len,
    ...
    pack_puback(uint8_t* buffer, uint16_t buffer_len,
                uint16_t topic_id, uint8_t msg_id, uint8_t return_code);
